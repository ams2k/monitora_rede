...
uses
  Classes, SysUtils, synsock, LResources, LCL, LCLIntf, dateutils, Forms,
...
type
  ...
  private
    ...
    DragEnabled : Boolean;
    mnuMain: TPopupMenu;
    mnuEditLabel: TMenuItem;
    mnuSep: TMenuItem;
    mnuDeleteLabel: TMenuItem;
    MyBackColor: TColor;

    iIDLabel: Integer;  //ID of label on database
    iIDNet: Integer;
    sText: String;
    iPosX: Integer;
    iPosY: Integer;
  ...
  public
    ...
    procedure InitLabel(idtit, idnet1, x, y: Integer);
  ...
  published
    ...
    property IDLabel: Integer read iIDLabel write iIDLabel;
    property IDNet: Integer read iIDNet write iIDNet;
...
uses
  u_frmlabel, u_dblabel, u_general;

procedure Register;
begin
  RegisterComponents('Misc', [TRotulo]);
end;
...
begin
   if ( (self.Left = Math.Max(0, X + self.Left - MouseDownPos.x)) and
       ...
       //if no changes in label position
       Exit;
  end;
...
begin
  if bPulse then
     begin
       ...
       if L.Visible then
          tmrLabel.Interval := 1500
       else
          tmrLabel.Interval := 300;
     end
  else
     L.Visible := True;
end;
...
var
  frm: TfrmLabel;
begin
  frm := TfrmLabel.Create(nil);
  frm.IDLabel := iIDLabel;
  frm.ShowModal;
...
begin
  c := TDBLabel.Create;
  c.SaveLabelPosition(iIDLabel, self.Left, self.Top);
  c.Free;
end;
...
procedure TRotulo.InitLabel(idtit, idnet1, x, y: Integer);
//Initialize label
var
  snum: string = '';
  i: Integer;
...
begin
  ...
  tmrLabel := TTimer.Create(self);
  tmrLabel.Interval := 500;
  tmrLabel.OnTimer  := @tmrLabelTimer;
  ...
  //Menu Item: mnuEditLabel
  mnuEditLabel := TMenuItem.Create(self);
  mnuEditLabel.Caption := 'Editar Rótulo';
  mnuEditLabel.Bitmap.LoadFromLazarusResource('pencil_16');
  mnuEditLabel.OnClick := @mnuEditLabelClick;
  //Menu Item: mnuSep
  mnuSep := TMenuItem.Create(self);
  mnuSep.Caption := '-';
  //Menu Item: mnuDeleteLabel
  mnuDeleteLabel := TMenuItem.Create(self);
  mnuDeleteLabel.Caption := 'Deletar Rótulo';
  mnuDeleteLabel.Bitmap.LoadFromLazarusResource('delete_16');
  mnuDeleteLabel.OnClick := @mnuDeleteLabelClick;
  //Add Items to mnuMain
  mnuMain.Items.Add(mnuEditLabel);
  mnuMain.Items.Add(mnuSep);
  mnuMain.Items.Add(mnuDeleteLabel);
...
procedure TRotulo.ControlMouseDown(Sender: TObject; Button: TMouseButton;
...
procedure TRotulo.ControlMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  DragEnabled  := False;
...
procedure TRotulo.ControlMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if DragEnabled then
  begin
...
procedure TRotulo.LMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TRotulo.LMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  DragEnabled := False;
  Screen.Cursor := crDefault;
